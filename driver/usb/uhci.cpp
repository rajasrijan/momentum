  ...
  public:
    ...
    void print_descriptor(transfer_descriptor *pDesc)
    {
        printf("USB Transfer status:\n\tactual_length: %d"
               ...
               pDesc->status.actual_length, pDesc->status.bit_stuff_error, pDesc->status.timeout_crc, pDesc->status.non_acknowledged,
...
static pci_device_id supportedDevices[] = {{(uint16_t)PCI_ANY_ID, (uint16_t)PCI_ANY_ID, (uint16_t)PCI_ANY_ID, (uint16_t)PCI_ANY_ID, PCI_BASE_CLASS_SERIAL,
                                            PCI_CLASS_SERIAL_USB, PCI_CLASS_SERIAL_USB_UHCI}};
